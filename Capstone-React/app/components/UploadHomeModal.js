...
// Helper function to detect if URI is a video or GIF
const isVideoOrGif = (uri) => {
  if (!uri || typeof uri !== 'string') return false;
  const path = uri.split('?')[0].toLowerCase();
  return /\.(mp4|mov|avi|mkv|gif)(\?.*)?$/i.test(path);
};
          ...
          <View style={styles.uploadModalContent}>
            ...
            >
              ...
              <TouchableOpacity style={styles.uploadButton} onPress={uploadPost} disabled={loading}>
                <Text style={styles.uploadButtonText}>{loading ? 'Posting...' : 'Post'}</Text>
              </TouchableOpacity>
            </ScrollView>
          </View>
        </View>
